...
public class 指し手生成Tests
{
    ...
    public void 竪行_縦スライドと横1升()
    {
        ...
        // 縦上: 段4不成り, 段3成り+不成り, 段2成り+不成り, 段1成り+不成り = 7手
        // 縦下: 段6,7,8 不成り × 3 (段9に先手玉がいない配置)
        // 横: (4,5)不成り, (6,5)不成り = 2手
        // 合計: 7 + 4 + 2 = 13手（先手玉(1,9)後手玉(9,1)は干渉なし）
        Assert.Equal(13, 手一覧.Count);
    ...
    public void 麒麟_斜め1升と縦横2升跳び()
    {
        ...
        // 斜め1升: (4,4),(6,4),(4,6),(6,6) = 4升（敵陣外→不成り×4）
        // 縦横2升: (5,3),(5,7),(3,5),(7,5) = 4升
        //   段3は敵陣→成り+不成り2手, 段7は不敵陣→1手
        //   列3,7は不敵陣→各1手
        // 合計: 4 + 2 + 1 + 1 + 1 = 9手
        Assert.Equal(9, 手一覧.Count);

        Assert.Contains(手一覧, 手 => 手.Get移動先.Equals(Sq(5, 3)) && 手.Is成り);
        Assert.Contains(手一覧, 手 => 手.Get移動先.Equals(Sq(5, 3)) && !手.Is成り);
        Assert.DoesNotContain(手一覧, 手 => 手.Get移動先.Equals(Sq(5, 2)));
    }
    ...
    public void 鳳凰_縦横1升と斜め2升跳び()
    {
        ...
        // 縦横1升: (5,4),(5,6),(4,5),(6,5) = 4升（敵陣外→不成り×4）
        // 斜め2升: (3,3),(7,3),(3,7),(7,7)
        //   (3,3)段3=敵陣→成り+不成り, (7,3)段3=敵陣→成り+不成り
        //   (3,7)(7,7) 不敵陣 → 各1手
        // 合計: 4 + 2 + 2 + 1 + 1 = 10手
